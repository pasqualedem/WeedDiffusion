...
class SegmentationNetwork(pl.LightningModule):
    ...
    def __init__(
        ...
        self.metric_train_iou = JaccardIndex(
            task="multiclass", num_classes=self.network.num_classes, average=None
        )
        self.metric_val_iou = JaccardIndex(
            task="multiclass", num_classes=self.network.num_classes, average=None
        )
        self.metric_test_iou = JaccardIndex(
            task="multiclass", num_classes=self.network.num_classes, average=None
        )
    ...
    def training_step(self, batch: dict, batch_idx: int) -> Dict[str, Any]:
        ...
        if "regular" in self.train_step_settings:
            ...
            loss_regular = self.compute_xentropy_loss(
                logits_regular, batch["anno"], mode="train", mask_keep=mask_keep_regular
            )
    ...
    def on_train_epoch_end(self, training_step_outputs: List) -> None:
        ...
        for key in dict_keys:
            if key.startswith("loss"):
                ...
                self.logger.experiment.add_scalar(f"{key}", {"train": loss_avg}, epoch)
        ...
        for class_index, iou_class in enumerate(iou_per_class):
            self.logger.experiment.add_scalar(
                f"iou_class_{class_index}", {"train": iou_class}, epoch
            )
        self.logger.experiment.add_scalar("mIoU", {"train": mIoU}, epoch)
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> Dict[str, Any]:
        if not self.val_step_settings:
            raise ValueError(
                "You need to specify the settings for the validation step."
            )
        ...
        self.validation_step_outputs.append(
            {"loss": loss, "logits": logits, "anno": batch["anno"]}
        )
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        # logging
        epoch = self.trainer.current_epoch
        self.logger.experiment.log_metrics({"val/loss": val_loss_avg}, epoch)
        ...
        for class_index, iou_class in enumerate(iou_per_class):
            self.logger.experiment.log_metrics(
                {f"val/iou_class_{class_index}": iou_class}, epoch
            )
        self.logger.experiment.log_metrics({"val/mIoU": mIoU}, epoch)
    ...
    def test_epoch_end(self, test_step_outputs: List) -> None:
        ...
        path_to_classwise_iou_dir = os.path.join(
            self.trainer.log_dir, "evaluation", "iou-classwise", f"epoch-{epoch:06d}"
        )
        save_iou_metric(iou_per_class, path_to_classwise_iou_dir)
        ...
        if current_epoch <= warm_up_epochs:
            ...
            lr_scale = pow(
                (
                    1
                    - (
                        (current_epoch - (warm_up_epochs + 1))
                        / (self.trainer.max_epochs - (warm_up_epochs + 1))
                    )
                ),
                3.0,
            )
    ...
    def configure_optimizers(
        self,
    ) -> Tuple[List[optim.Optimizer], List[optim.lr_scheduler.LambdaLR]]:
